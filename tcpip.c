   ...
   // #include <LPC22xx.h>      // Keil: Register definition file for LPC2294  
    ...
    // Keil: Timer 0 is used for TCP retransmission control  
      T0MR0 = 3930000;                            // 262mSec   
      T0MCR = 3;                                  // Interrupt and Reset on MR0   
      T0TCR = 1;                                  // Timer0 Enable   
      VICVectAddr0 = (unsigned long)TCPClockHandler;// set interrupt vector in 0   
        ...
        {  
          if (TCPFlags & TCP_ACTIVE_OPEN)            // stack has to open a connection?  
            if (TCPFlags & IP_ADDR_RESOLVED)         // IP resolved?  
              ...
              { 
            	  uprintf("tcp connection send syn\n");
    ...
    {  
      ...
      switch (ReadFrameBE91111())                     // get frame type  
      {  
    ...
    {  
      ...
      uprintf("processtcpframe tcpstate %d\n",TCPStateMachine);
      switch (TCPStateMachine)                                 // implement the TCP state machine  
      {  
        case CLOSED :  
        {  
        	uprintf("closed tcp state TCPCode %d\n",TCPCode);
        ...
        case LISTENING :  
        {  
        	uprintf("listening tcp state TCPCode %d\n",TCPCode);
          ...
          {  
            ...
            {
            	uprintf("listening and tcp_codeSyn\n");
        ...
        {  
          ...
          {  
            ...
            {  
              ...
              {  
            	  uprintf("last ack state TCPCode %d\n",TCPCode);
                TCPStateMachine = CLOSED;  
                TCPFlags = 0;                        // reset all flags, stop retransmission...  
                SocketStatus &= SOCK_DATA_AVAILABLE; // clear all flags but data available  
                break;  
    ...
    {  
      ...
      *(unsigned short *)&TxFrame2[TCP_URGENT_OFS] = 0;  
      ...
      if (TCPCode & TCP_CODE_SYN)                    // if SYN, we want to use the MSS option  
      {  
    	  uprintf(" 764 TCPCode & TCP_CODE_SYN\n");
